...
const TestimonialPopup: React.FC<{ testimonial: Testimonial; onClose: () => void }> = ({
  ...
  <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div className="relative w-[90%] max-w-[600px] rounded-2xl p-8 bg-[#F0F0F0] dark:bg-[#0b0b0d] border border-[rgba(37,99,235,0.3)] text-black dark:text-white shadow-2xl">
      {/* Close button */}
      <button
        onClick={onClose}
        className="absolute top-4 right-4 text-black dark:text-white text-2xl font-bold hover:text-red-500"
      >
        ×
      </button>
      ...
      <div className="flex items-center justify-between pt-5 mt-5 border-t border-[rgba(37,99,235,0.3)] dark:border-[rgba(255,255,255,0.2)]">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="font-bold text-sm md:text-base">{testimonial.clientName}</div>
...
// -------- Testimonial Card --------
const TestimonialCard: React.FC<{ testimonial: Testimonial; onReadMore: () => void }> = ({
  testimonial,
  onReadMore,
}) => (
  <div className="relative min-w-[280px] max-w-[280px] md:min-w-[360px] md:max-w-[360px] lg:min-w-[400px] lg:max-w-[400px] h-[380px] flex-shrink-0 group">
    <div className="relative h-full rounded-2xl p-6 md:p-7 lg:p-8 transition-all duration-500 hover:-translate-y-2 border border-[rgba(37,99,235,0.2)] backdrop-blur-[10px] bg-[#F0F0F0] dark:bg-[#F0F0F01A] text-black dark:text-white hover:bg-gradient-to-br hover:from-[#2563eb] hover:to-[#1d4ed8] hover:text-white dark:hover:text-white flex flex-col">
      ...
      {/* Footer */}
      <div className="flex items-center justify-between gap-3.5 pt-6 mt-5 border-t-[1.5px] border-[rgba(37,99,235,0.3)] dark:border-[rgba(255,255,255,0.2)] transition-all duration-300 group-hover:border-[rgba(255,255,255,0.5)]">
        <div className="flex items-center gap-3.5">
          <div
  ...
  return (
    <section className="relative py-20 md:py-24 lg:py-28 overflow-hidden bg-[#ffffff] dark:bg-[#0b0b0d] text-black dark:text-white transition-colors duration-500">
      <div className="text-center mb-12 md:mb-16 animate-fadeIn px-5">
        <h2 className="text-4xl md:text-5xl lg:text-[56px] font-black mb-5 leading-tight tracking-tight text-center text-black dark:text-white whitespace-pre-line">
          {`Happy Words From\nHappy Customer`}
        </h2>
      </div>
      ...
      <div className="w-full overflow-hidden">
        ...
        <div className="flex gap-4 md:gap-5 mb-4 md:mb-5 animate-scrollLeft hover:pause">
          {testimonialsData.row1.map((t) => (
            <TestimonialCard key={t.id} testimonial={t} onReadMore={() => setPopupTestimonial(t)} />
          ))}
          {testimonialsData.row1.map((t) => (
            <TestimonialCard key={`dup-${t.id}`} testimonial={t} onReadMore={() => setPopupTestimonial(t)} />
          ))}
        </div>
      ...
      {/* Render popup OUTSIDE the rows */}
      {popupTestimonial && (
        <TestimonialPopup testimonial={popupTestimonial} onClose={() => setPopupTestimonial(null)} />
      )}
