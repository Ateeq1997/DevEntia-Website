    ...
    >
      ...
      <div
        ...
        className="scroll-container flex gap-4 md:gap-6 items-stretch w-full mt-6 
                   overflow-x-auto lg:overflow-x-visible scrollbar-hide snap-x snap-mandatory 
                   scroll-smooth cursor-grab active:cursor-grabbing md:cursor-default md:snap-none"
      >
        {portfolioData.map((card) => (
          <Link
            key={card.id}
            href={`/Casestudy/${card.id}`}
            className="flex-shrink-0 snap-center w-[85%] sm:w-[45%] md:w-[40%] lg:w-[24%]
